...
public class FileDownloadUploadUtils {
    ...
    public static String serialize(List<? extends Person> people, MapMessage.MapFormat format) {
        return format.equals(JSON) ? serialize(people, JSON_MAPPER) : serialize(people, XML_MAPPER);
    }

    private static String serialize(List<? extends Person> people, ObjectMapper objectMapper) {
        try {
